...
// The program address.
declare_id!("6DNF4tFLynLJhJh78dCNkft9A5Sd49cmypD2SZ4Hfu7S");
...
pub mod plugin_boilerplate {
    ...
    pub fn create_voter_weight_record(ctx: Context<CreateVoterWeightRecord>, voter_weight_record_bump: u8) -> Result<()> {
        instructions::create_voter_weight_record(ctx, voter_weight_record_bump)
    }

    pub fn update_voter_weight_record(ctx: Context<UpdateVoterWeightRecord>, voter_weight_record_bump: u8) -> Result<()> {
        instructions::update_voter_weight_record(ctx, voter_weight_record_bump)
    }
